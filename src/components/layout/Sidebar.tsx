...
export function Sidebar({ activePanel, onPanelChange }: SidebarProps) {
  ...
  return (
    <div
      className={`h-full bg-slate-900/95 backdrop-blur-sm border-r border-slate-700/50 flex flex-col transition-all duration-300 ${
        collapsed ? 'w-16' : 'w-56'
      }`}
    >
      {/* Logo */}
      <div className="p-4 border-b border-slate-700/50 flex items-center gap-3">
        ...
        {!collapsed && (
          <span className="font-bold text-white text-lg tracking-tight">GeoVerse</span>
        )}
      </div>
      ...
      <button
        ...
        className="p-3 border-t border-slate-700/50 text-slate-500 hover:text-white transition-colors flex justify-center"
      >
